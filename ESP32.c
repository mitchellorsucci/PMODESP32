...
static int bufferedUartSend(Uart * devicePtr, u8 * data, int length) {
    ...
    for(int i = 0; i < length; i++) {
    	// Wait til the device is no longer sending
        // while(XUartLite_IsSending(devicePtr));
    	// XUartLite_Send(devicePtr, cursor, UART_BUF_SIZE);
        // bufSent++;
        // cursor += UART_BUF_SIZE;
        while(XUartLite_IsSending(devicePtr));
    ...
    // Wait til the device is no longer sending
    // while(XUartLite_IsSending(devicePtr));
    // int remaining = length - (bufSent * UART_BUF_SIZE);
    // XUartLite_Send(devicePtr, cursor, remaining);
    return XST_SUCCESS;
}
...
int  getConnectionStatus(Uart * devicePtr) {
    ...
    // char * type should be one of "TCP", "UDP", or "SSL"
